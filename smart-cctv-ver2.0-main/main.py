...
window = tk.Tk() #creates a Tkinter window object
window.title("Smart cctv")
window.iconphoto(False, tk.PhotoImage(file='mn.png'))
window.geometry('1080x700')


frame1 = tk.Frame(window) #creates a frame to hold the elements of the GUI.

label_title = tk.Label(frame1, text="Smart cctv Camera") #creates a label widget to display the title of the application
label_font = font.Font(size=35, weight='bold',family='Helvetica')
label_title['font'] = label_font
label_title.grid(pady=(10,10), column=2)


icon = Image.open('icons/spy.png') #script loads an image 
icon = icon.resize((150,150), Image.ANTIALIAS) #resizing the image
icon = ImageTk.PhotoImage(icon)
label_icon = tk.Label(frame1, image=icon) 
label_icon.grid(row=1, pady=(5,10), column=2) #resized image is then displayed in a label


#script loads several image files, resizes them, and converts them to Tkinter-compatible PhotoImage objects 
...
frame1.pack()
window.mainloop() #continuously updates the GUI and handles user interactions
